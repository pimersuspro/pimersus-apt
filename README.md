...
The most up-to-date version is **4.5.6**, distributed via **APT repositories** and receiving regular updates.
...
```bash
sudo dpkg -i pimersus-browser-slim_4.5.6_i386.deb
...
```bash
sudo dpkg -i pimersus-browser_4.5.6_i386.deb
sudo apt-get -f install
```
...
```bash
sudo dpkg -i pimersus-browser-debian12_4.5.6_amd64.deb
sudo apt-get -f install
```
...
```bash
sudo dpkg -i pimersus-browser-debian13_4.5.6_amd64.deb
sudo apt-get -f install
```
...
```bash
sudo dpkg -i pimersus-browser-db13-boosted_4.5.6_amd64.deb
sudo apt-get -f install
```
...
```bash
sudo dpkg -i pimersus-browser-db13-mcbkpro_4.5.6_amd64.deb
sudo apt-get -f install
```
